...
**Time/Space:** As written there is no memoization — only a backtracking `visiting` set — so a shared prerequisite reachable via multiple paths is re-evaluated once per path. For a DAG with shared prerequisites (diamond dependencies) the number of `eval` calls equals the number of distinct paths through the graph, which is **exponential in the worst case** (≈ O(2^depth)), not O(V+E); recursion depth is still bounded by `visiting`. To make it linear — **O(V + E) per top-level evaluation** — add a per-evaluation result cache (`Map<String,Boolean> memo` alongside `visiting`) so each flag is evaluated at most once; this is safe in a DAG because a flag's result is path-independent, and `visiting` still catches cycles before any node is cached. **Edge cases:** (1) a cycle is a configuration error, not a runtime expectation — fail loud (platforms reject cycles at save time). (2) Remove the key from `visiting` on the way out (not a global `visited` set) so a diamond dependency evaluated via two paths isn't falsely flagged as a cycle. (3) A missing prerequisite flag should be treated as *not satisfied* (fail safe), not as an exception that 500s the request.
